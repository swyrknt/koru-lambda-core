...
async function main() {
    ...
    console.log(`  Network root: ${agent.currentRoot().substring(0, 16)}...\n`);
...
async function main() {
    ...
    // Benchmark 1: Core Synthesis
    console.log('═══ Benchmark 1: Core Synthesis ═══');
    const engine = new WasmEngine();
    const d0 = engine.d0Id();
    const d1 = engine.d1Id();

    results.synthesis = await benchmark(
        'Core Synthesis',
        () => engine.synthesize(d0, d1),
        10000
    );
    ...
    results.leaderElection = await benchmark(
        'Leader Election',
        () => network.getLeader(),
        100000
    );

    // Benchmark 3: Batch Validation
    console.log('═══ Benchmark 3: Batch Validation ═══');
    const validator = new WasmValidator(engine);
    let nonce = 0;

    results.batchValidation = await benchmark(
        'Batch Validation (1 tx)',
        () => {
            const batch = JSON.stringify({
                transactions: [{ nonce: nonce++, data: [1, 2, 3] }],
                previous_root: validator.currentRoot()
            });
    ...
    // Benchmark 4: Two-Stage Commitment
