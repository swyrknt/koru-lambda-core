# Testing Standards - Koru Lambda Core (Rust)

## Philosophy

The Koru Lambda Core test suite employs **falsification methodology**. Tests are designed to attack hypotheses, not validate them. A hypothesis is considered sustained only when rigorous attempts to falsify it fail.
...
1. **Hypothesis**: Clear statement of the expected property or behavior
...
| **Degree Distribution** | Test for structural patterns | Histogram of node degrees |
...
pub use koru_lambda_core::{Distinction, DistinctionEngine};
```
