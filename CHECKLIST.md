...
- [ ] Crate-level lint floor at `lib.rs`: `#![warn(clippy::unwrap_used, clippy::expect_used, clippy::must_use_candidate, clippy::missing_const_for_fn)]`
...
- [ ] `Cargo.toml` `[dependencies]` adds: `bytemuck = "1"` (Pod derive), `parking_lot = "0.12"` (FFI Mutex), `console_error_panic_hook = "0.1"` (optional under `wasm` feature). thiserror / dashmap / sha2 / serde / lru / rayon / hex already in dev.
...
- [ ] **Differential testing:**
  - **Hash-algorithm cross-check** (rust-craftsman round-2): a ~50 LOC
    naive reference impl `synthesize_ref(a, b)` using `blake3::Hasher`
    (different algorithm, same axioms) + canonical `[u8;16]` byte ordering
    + leading 16 bytes. Run 1M random `(a, b)` pairs through both impls;
    they must NEVER produce the same child bytes (different hash) BUT must
    agree on commutativity (`synthesize_ref(a,b) == synthesize_ref(b,a)`)
    and irreflexivity (`synthesize_ref(a,a) == a`). Falsifies canonical-
    ordering inversion, prefix-truncation drift, and irreflexivity short-
    circuit removal — bug classes the same-sha2 differential and the
    cross-engine probe cannot catch.
...
- [ ] Memory per distinction ≤ 140 B at 1M scale (dhat live-heap, steady state; arithmetic target ~134 B + ~6 B headroom)
...
- [ ] **Pin the exp18 corpus** at `/tests/corpora/exp18.log` — capture the deterministic synthesis log produced by running the workload module above with the pinned constants, then freeze the file. Step 4 replays from this corpus, not from a re-run, so the ρ ≥ 0.985 gate is bit-exact reproducible across machines.
...
- [ ] Memory per distinction at 1M scale ≤ 140 B (dhat)
...
- Anything not gated by a Step 1-5 gate above. If it's worth doing for v2.0, it's a checklist item; if it's not on this list, it's not in v2.0.
