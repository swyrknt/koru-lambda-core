...
fn test_commitment_multi_node_consensus() {
    ...
    let mut agents: Vec<NetworkAgent> = (0..NUM_NODES)
    ...
    let result = agents[0].finalize_batch(
        batch.clone(),
    ...
    assert!(
        result.is_ok(),
        "Leader failed to finalize batch: {:?}",
        result
    );
    ...
    assert_ne!(
        previous_root, final_root,
        "INVARIANT VIOLATION: State did not advance after batch"
    );
...
fn test_falsify_byzantine_leader_invalid_commitment() {
    ...
    assert!(
        !is_valid,
        "FALSIFICATION FAILED: Validator accepted commitment with invalid nonce"
    );
...
fn test_falsify_commitment_hash_mismatch() {
    ...
    // Create commitment
    let commitment = agent
        .propose_commitment(original_batch.clone(), &engine)
    ...
    let result = agent.finalize_batch(
        tampered_batch,
        commitment.commitment_hash,
        &engine,
    );
...
fn test_commitment_across_epoch_boundary() {
    ...
    let commitment = leader_node
        .propose_commitment(batch_epoch_0.clone(), &engine)
        .expect("Propose at epoch 0");
    ...
    assert!(
        !is_valid,
        "INVARIANT VIOLATION: Old epoch commitment accepted at new epoch"
    );
...
fn test_falsify_concurrent_leader_commitments() {
    ...
    for (idx, agent) in agents.iter_mut().enumerate() {
        ...
        let commitment = agent
            .propose_commitment(batch, &engine)
            .expect("Each node proposes");
...
fn test_network_partition_recovery() {
    ...
    let mut partition_a: Vec<NetworkAgent> = (0..3)
        .map(|_| NetworkAgent::new(&engine))
        .collect();

    let mut partition_b: Vec<NetworkAgent> = (0..3)
        .map(|_| NetworkAgent::new(&engine))
        .collect();
    ...
    let partition_a_roots: Vec<String> = partition_a
        .iter()
    ...
    let partition_b_roots: Vec<String> = partition_b
        .iter()
        .map(|a| a.get_current_root().id().to_string())
        .collect();
...
fn test_commitment_high_throughput_stress() {
    ...
    for batch_idx in 0..NUM_BATCHES {
        ...
        if (batch_idx + 1) % 25 == 0 {
            println!("  Progress: {}/{} batches, {} transactions", batch_idx + 1, NUM_BATCHES, total_txs);
        }
    }

    assert_eq!(
        successful, NUM_BATCHES,
        "Some batches failed: {}/{}",
        successful, NUM_BATCHES
    );
...
fn test_falsify_commitment_replay_attack() {
    ...
    assert!(
        !is_valid,
        "FALSIFICATION FAILED: System accepted replayed commitment"
    );
