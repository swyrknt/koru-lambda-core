...
/// (Resources proportional to Utility) using scale-free topology.
///
/// The compactor should:
/// 1. Identify high-utility distinctions (hubs) via S.I.S.
/// 2. Archive low-utility distinctions (power-law tail)
/// 3. Preserve structural integrity of the graph
///
...
use koru_lambda_core::{
    Canonicalizable, CompactionAction, DistinctionEngine, LocalCausalAgent, StructuralCompactor,
};
use std::sync::Arc;

/// Falsification Test: Random Compaction Strategy
///
/// Hypothesis: The Structural Compactor uses scale-free topology to achieve
/// efficient compression. By preserving high-degree hubs and archiving
/// low-degree nodes, it maintains graph coherence while reducing resource
/// footprint.
///
/// Falsifies if: Compaction fails to identify structural hierarchy, archiving
/// nodes randomly without regard to S.I.S., leading to poor compression ratios
/// (<2x) or loss of graph connectivity.
///
/// Measurement:
...
fn test_falsify_random_compaction() {
    ...
    println!("    COLD archival: {:.1}%", (stats.cold_count as f64 / initial_count as f64) * 100.0);
...
fn test_falsify_statistics_inaccuracy() {
    ...
    assert_eq!(
        ...
        total_distinctions, accounted, stats.hot_count, stats.warm_count, stats.cold_count
    );
