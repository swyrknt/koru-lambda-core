...
fn test_falsify_information_independence() {
    ...
    let (center_a_id, center_b_id) = loop {
        ...
        if ego_a.is_disjoint(&ego_b) && ego_a.len() >= 40 && ego_b.len() >= 40 {
            ...
            println!(
                "    Subprocess A: {} nodes (center: {})",
                ego_a.len(),
                id_a
            );
            println!(
                "    Subprocess B: {} nodes (center: {})",
                ego_b.len(),
                id_b
            );
            break (id_a, id_b);
    ...
    for step in 0..observation_steps {
        ...
        let center_a_idx = current_graph
            .node_identifiers()
            .find(|&n| current_graph[n] == center_a_id);
        let center_b_idx = current_graph
            .node_identifiers()
            .find(|&n| current_graph[n] == center_b_id);
