/// Falsification Test: Conscious Dynamics
///
...
mod conscious_helpers {
    ...
    pub fn clustering_coefficients(
        ...
        graph
            .node_identifiers()
            .map(|n| (n, node_clustering_coefficient(graph, n)))
            .collect()
    }
    ...
    pub fn subprocess_coherence(
        ...
        let sum: f64 = subprocess_nodes
            .iter()
            .filter_map(|n| clustering.get(n))
            .sum();
...
fn evolve_locally(engine: &mut DistinctionEngine, steps: usize, rng: &mut StdRng) {
    for _step in 0..steps {
        ...
        let graph = conscious_helpers::build_graph(engine);
        ...
        let b = if let Some(&idx_a) = a_idx {
            ...
            if !neighborhood_distinctions.is_empty() {
                ...
                let others: Vec<_> = distinctions.iter().filter(|d| d.id() != a.id()).cloned().collect();
                if others.is_empty() {
                    continue;
                }
...
fn evolve_subprocess(
    ...
    for _step in 0..steps {
        let all_distinctions: Vec<_> = engine
            .get_state_snapshot()
            .0
            .iter()
            .cloned()
        ...
        let b_options: Vec<_> = subprocess_distinctions
            .iter()
            .filter(|d| d.id() != a.id())
            .cloned()
            .collect();
...
#[test]
fn test_falsify_passive_labeling() {
    // ============================================================
    // SETUP
    // ============================================================
    println!("\nTest: Passive Labeling Falsification");
    println!("  Testing for conscious influence on dynamics...");
    ...
    let initial_graph = conscious_helpers::build_graph(&engine);
    ...
    assert!(
        node_count >= 100,
        "Graph too small for robust testing: {} nodes",
        node_count
    );
    ...
    let all_clustering = conscious_helpers::clustering_coefficients(&initial_graph);
    ...
    for node_idx in initial_graph.node_identifiers() {
        let ego = conscious_helpers::ego_graph(&initial_graph, node_idx, 2);
        let coherence = conscious_helpers::subprocess_coherence(&all_clustering, &ego);
    ...
    let (low_coherence_center, low_coherence_initial) =
        *subprocess_coherences.last().unwrap();

    println!(
        "    High-coherence subprocess: {:.4}",
        high_coherence_initial
    );
    println!(
        "    Low-coherence subprocess: {:.4}",
        low_coherence_initial
    );
    ...
    let high_subprocess_nodes = conscious_helpers::ego_graph(&initial_graph, high_coherence_center, 2);
    let low_subprocess_nodes = conscious_helpers::ego_graph(&initial_graph, low_coherence_center, 2);

    let high_subprocess_ids: HashSet<String> = high_subprocess_nodes
        .iter()
        .map(|&idx| initial_graph[idx].clone())
        .collect();

    let low_subprocess_ids: HashSet<String> = low_subprocess_nodes
        .iter()
        .map(|&idx| initial_graph[idx].clone())
        .collect();

    println!(
        "    High-coherence subprocess: {} nodes",
        high_subprocess_ids.len()
    );
    println!(
        "    Low-coherence subprocess: {} nodes",
        low_subprocess_ids.len()
    );
    ...
    // ============================================================
    let final_graph = conscious_helpers::build_graph(&engine);
    let final_clustering = conscious_helpers::clustering_coefficients(&final_graph);

    let final_high_subprocess = conscious_helpers::ego_graph(&final_graph, high_coherence_center, 2);
    let final_low_subprocess = conscious_helpers::ego_graph(&final_graph, low_coherence_center, 2);

    let high_coherence_final =
        conscious_helpers::subprocess_coherence(&final_clustering, &final_high_subprocess);
    let low_coherence_final =
        conscious_helpers::subprocess_coherence(&final_clustering, &final_low_subprocess);
    ...
    println!(
        "  Magnitude ratio: {:.2}x",
        delta_high.abs() / delta_low.abs().max(0.0001)
    );
