...
use crate::{DistinctionEngine, Distinction};
use crate::subsystems::{
    LocalCausalAgent,  // Used to access subsystem methods
    NetworkAgent, PeerIdentity,
    TransactionBatch,
    ConsensusValidator,
    CommitmentAgent, BatchCommitment,
};
...
impl WasmEngine {
    ...
    pub fn new() -> Self {
        Self {
            inner: Arc::new(DistinctionEngine::new()),
        }
    }
...
impl WasmNetworkAgent {
    ...
    pub fn check_commitment(&self, hash_hex: &str, nonce: u64, epoch: u64) -> Result<bool, JsValue> {
...
mod tests {
    ...
    fn test_wasm_synthesis_determinism() {
        ...
        // Synthesize via WASM
        let wasm_result = wasm_engine.synthesize(&d0_id, &d1_id)
            .expect("WASM synthesis should succeed");

        // Synthesize via native
        let native_result = native_engine.synthesize(
            native_engine.d0(),
            native_engine.d1()
        );
    ...
    fn test_wasm_axiom_symmetry() {
        ...
        // Axiom: Symmetry - order independence
    ...
    fn test_wasm_axiom_irreflexivity() {
        ...
        // Axiom: Irreflexivity - self-synthesis yields self
    ...
    fn test_wasm_network_agent_local_causal_compliance() {
        ...
        let new_root = agent.join_peer("validator_0")
            .expect("Peer join should succeed");

        // LocalCausalAgent contract: ΔNew ≠ ΔOld (action changes state)
        assert_ne!(new_root, initial_root,
            "NetworkAgent didn't update root after action");
    ...
    fn test_wasm_validator_local_causal_compliance() {
        ...
        let new_root = validator.validate_batch(&batch.to_string())
            .expect("Valid batch should succeed");

        // LocalCausalAgent contract: ΔNew ≠ ΔOld
        assert_ne!(new_root, initial_root,
            "Validator didn't update root after validation");
    ...
    fn test_wasm_commitment_protocol_integrity() {
        ...
        let hash = agent.propose_commitment(&batch)
            .expect("Commitment proposal should succeed");
        ...
        let is_valid = agent.check_commitment(&hash, 0, 0)
            .expect("Commitment check should succeed");
    ...
    fn test_wasm_commitment_hash_tampering_rejected() {
        ...
        // Propose commitment
        let _correct_hash = agent.propose_commitment(&batch)
            .expect("Commitment proposal should succeed");
    ...
    fn test_wasm_commitment_nonce_enforcement() {
        ...
        let is_valid = agent.check_commitment(&hash, 999, 0)
            .expect("Check should not error");

        assert!(!is_valid,
            "FALSIFICATION FAILED: Accepted commitment with wrong nonce");
    }
    ...
    fn test_wasm_hex_encoding_correctness() {
        ...
        assert_eq!(&decoded[..], &original[..],
            "Hex encoding/decoding corrupted data");
    }
    ...
    fn test_wasm_deterministic_leader_election() {
        ...
        assert_eq!(leader1, leader2,
            "FALSIFICATION FAILED: Leader election is non-deterministic");
    }
}
