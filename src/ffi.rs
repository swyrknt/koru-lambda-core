...
///
/// Design Principles:
/// - **Zero-Cost Abstraction**: Direct pointer operations, no overhead
/// - **Memory Safety**: Caller owns all allocated memory, explicit free functions
/// - **Thread Safety**: All operations are thread-safe via Arc/DashMap
/// - **Determinism**: All functions are pure or explicitly mutate via pointers

use crate::{
    DistinctionEngine, NetworkAgent, PeerIdentity,
    TransactionBatch, BatchCommitment,
    ConsensusValidator,
};
...
pub unsafe extern "C" fn koru_agent_propose_commitment(
    ...
    std::ptr::copy_nonoverlapping(
        commitment.commitment_hash.as_ptr(),
        out_commitment,
        32,
    );
...
pub unsafe extern "C" fn koru_agent_propose_batch(
    ...
    #[allow(deprecated)]
    let result = match agent.propose_batch(batch, &engine_arc) {
        Ok(_) => KORU_SUCCESS,
        Err(_) => KORU_ERROR_BATCH_REJECTED,
    };
...
pub unsafe extern "C" fn koru_agent_get_leader(agent: *const KoruAgent) -> *mut c_char {
    ...
    match agent.get_current_leader() {
        Some(leader) => {
            match CString::new(leader.id.clone()) {
                Ok(s) => s.into_raw(),
                Err(_) => std::ptr::null_mut(),
            }
        },
